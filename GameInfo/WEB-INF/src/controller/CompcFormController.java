...
@Controller
public class CompcFormController {
	...
	@RequestMapping(value = "com_pc_write.info", method = RequestMethod.GET)
	public ModelAndView bbs_write() {

		ModelAndView model = new ModelAndView();
		model.setViewName("community/com_pc_write");

		return model;
	}

	@SuppressWarnings("unused")
	@RequestMapping(value = "/com_pc_write.info", method = RequestMethod.POST)
	public ModelAndView submit(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		BbsBean bbsbean = new BbsBean();
		String saveFolder = "C:/JavaDevProject/Portfolilo/GameInfo/Compcupload";
		...
		this.bbsService.insertcompc(bbsbean);
		response.sendRedirect("com_pc_list.info");
		return null;
	}

	// �섏젙

	@RequestMapping(value = "/com_pc_edit.info", method = RequestMethod.GET)
	public ModelAndView editGet(HttpServletRequest request,
		...
		bbsbean = (BbsBean) this.bbsService.compccheckpwd(no);
		...
		if (session.getAttribute("userstate") == null) {	
			...
			ModelAndView model = new ModelAndView();
			model.setViewName("community/com_pc_edit");
	...
	@RequestMapping(value = "/com_pc_edit.info", method = RequestMethod.POST)
	public ModelAndView edit(HttpServletRequest request,
		...
		this.bbsService.editcompc(bbsbean);
		response.sendRedirect("com_pc_list.info");
		return null;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "com_pc_list.info", method = RequestMethod.GET)
	public ModelAndView bbs_List(HttpServletRequest request,
		...
		int listcount = this.bbsService.getcompcListCount();

		bbslist = bbsdao.getcompcList(page, limit);
		...
		model2.setViewName("../sub03_03");
		return model2;

	}

	@RequestMapping(value = "/com_pc_find.info", method = RequestMethod.POST)
	public ModelAndView bbs_find(HttpServletRequest request,
		...
		int listcount = this.bbsService.getcompcFindCount();
		...
		if (find_field.equals("nickname")) {
			findList = this.bbsService.compcFind(find_name, page, limit);
		} else {
			findList = this.bbsService.compcFind2(find_name, page, limit);
		}
		...
		model.setViewName("community/com_pc_find");
		return model;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/com_pc_cont.info", method = RequestMethod.GET)
	public ModelAndView bbs_view(HttpServletRequest request,
		...
		// �볤�
		int listcount = this.bbsService.getcompcComCount(no);
		List comlist = new ArrayList();

		request.setAttribute("listcount", listcount);

		comlist = bbsdao.getcompcComList(no);
		...
		bbsbean = (BbsBean) bbsdao.contcompc(no);

		request.setAttribute("bbsdata", bbsbean);
		request.setAttribute("comdata", combean);

		ModelAndView model2 = new ModelAndView();
		model2.setViewName("community/com_pc_cont");
		return model2;
	}

	@RequestMapping(value = "/com_pc_del.info", method = RequestMethod.GET)
	public ModelAndView bbs_del(HttpServletRequest request,
		...
		if (session.getAttribute("userstate") == null) {	
			...
			out.println("</script>");
				...
				&& session.getAttribute("userstate").equals("admin")) {
			this.bbsService.delcompc(no, checkpwd);
			this.bbsService.compcComDelAll(no);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/com_pc_list.info");
			
			return model;
			
		} else if (checkpwd.equals(pwd)) {
			this.bbsService.delcompc(no, pwd);
			this.bbsService.compcComDelAll(no);
	...
	@RequestMapping(value = "/com_pc_com.info", method = RequestMethod.POST)
	public ModelAndView bbs_com(HttpServletRequest request,
		...
		if (nickname == null) {
			...
			this.bbsService.compcCom(combean);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/com_pc_cont.info?no=" + no);
			return model;
		}
		return null;
	}

	@RequestMapping(value = "/com_pc_comdel.info", method = RequestMethod.GET)
	public ModelAndView bbs_comdel(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {

		int bbsno = Integer.parseInt(request.getParameter("bbsno"));
		int comno = Integer.parseInt(request.getParameter("comno"));
		this.bbsService.compcComDel(bbsno, comno);

		ModelAndView model = new ModelAndView();
		model.setViewName("redirect:/com_pc_cont.info?no=" + bbsno);
		return model;
	}

	@RequestMapping(value = "/com_pc_recom.info", method = RequestMethod.GET)
