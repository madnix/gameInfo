...
@Controller
public class CompsFormController {
	...
	@RequestMapping(value = "com_ps_write.info", method = RequestMethod.GET)
	public ModelAndView bbs_write() {

		ModelAndView model = new ModelAndView();
		model.setViewName("community/com_ps_write");

		return model;
	}

	@SuppressWarnings("unused")
	@RequestMapping(value = "/com_ps_write.info", method = RequestMethod.POST)
	public ModelAndView submit(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		BbsBean bbsbean = new BbsBean();
		String saveFolder = "C:/JavaDevProject/Portfolilo/GameInfo/Compsupload";
		...
		this.bbsService.insertcomps(bbsbean);
		response.sendRedirect("com_ps_list.info");
		return null;
	}

	// 수정

	@RequestMapping(value = "/com_ps_edit.info", method = RequestMethod.GET)
	public ModelAndView editGet(HttpServletRequest request,
		...
		bbsbean = (BbsBean) this.bbsService.compscheckpwd(no);
		...
		if (session.getAttribute("userstate") == null) {	
			...
			ModelAndView model = new ModelAndView();
			model.setViewName("community/com_ps_edit");
	...
	@RequestMapping(value = "/com_ps_edit.info", method = RequestMethod.POST)
	public ModelAndView edit(HttpServletRequest request,
		...
		this.bbsService.editcomps(bbsbean);
		response.sendRedirect("com_ps_list.info");
		return null;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "com_ps_list.info", method = RequestMethod.GET)
	public ModelAndView bbs_List(HttpServletRequest request,
		...
		int listcount = this.bbsService.getcompsListCount();

		bbslist = bbsdao.getcompsList(page, limit);
		...
		model2.setViewName("../sub03_01");
		return model2;

	}

	@RequestMapping(value = "/com_ps_find.info", method = RequestMethod.POST)
	public ModelAndView bbs_find(HttpServletRequest request,
		...
		int listcount = this.bbsService.getcompsFindCount();
		...
		if (find_field.equals("nickname")) {
			findList = this.bbsService.compsFind(find_name, page, limit);
		} else {
			findList = this.bbsService.compsFind2(find_name, page, limit);
		}
		...
		model.setViewName("community/com_ps_find");
		return model;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/com_ps_cont.info", method = RequestMethod.GET)
	public ModelAndView bbs_view(HttpServletRequest request,
		...
		// 댓글
		int listcount = this.bbsService.getcompsComCount(no);
		List comlist = new ArrayList();

		request.setAttribute("listcount", listcount);

		comlist = bbsdao.getcompsComList(no);
		...
		bbsbean = (BbsBean) bbsdao.contcomps(no);

		request.setAttribute("bbsdata", bbsbean);
		request.setAttribute("comdata", combean);

		ModelAndView model2 = new ModelAndView();
		model2.setViewName("community/com_ps_cont");
		return model2;
	}

	@RequestMapping(value = "/com_ps_del.info", method = RequestMethod.GET)
	public ModelAndView bbs_del(HttpServletRequest request,
		...
		if (session.getAttribute("userstate") == null) {	
			...
			out.println("</script>");
				...
				&& session.getAttribute("userstate").equals("admin")) {
			this.bbsService.delcomps(no, checkpwd);
			this.bbsService.compsComDelAll(no);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/com_ps_list.info");
			return model;
		} else if (checkpwd.equals(pwd)) {
			this.bbsService.delcomps(no, pwd);
			this.bbsService.compsComDelAll(no);
	...
	@RequestMapping(value = "/com_ps_com.info", method = RequestMethod.POST)
	public ModelAndView bbs_com(HttpServletRequest request,
		...
		if (nickname == null) {
			...
			this.bbsService.compsCom(combean);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/com_ps_cont.info?no=" + no);
			return model;
		}
		return null;
	}

	@RequestMapping(value = "/com_ps_comdel.info", method = RequestMethod.GET)
	public ModelAndView bbs_comdel(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {

		int bbsno = Integer.parseInt(request.getParameter("bbsno"));
		int comno = Integer.parseInt(request.getParameter("comno"));
		this.bbsService.compsComDel(bbsno, comno);

		ModelAndView model = new ModelAndView();
		model.setViewName("redirect:/com_ps_cont.info?no=" + bbsno);
		return model;
	}

	@RequestMapping(value = "/com_ps_recom.info", method = RequestMethod.GET)
