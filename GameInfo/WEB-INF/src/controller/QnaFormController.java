...
public class QnaFormController {
	...
	@RequestMapping(value = "qna_write.info", method = RequestMethod.GET)
	public ModelAndView bbs_write() {

		ModelAndView model = new ModelAndView();
		model.setViewName("qna/qna_write");

		return model;
	}

	@SuppressWarnings("unused")
	@RequestMapping(value = "/qna_write.info", method = RequestMethod.POST)
	public ModelAndView submit(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		BbsBean bbsbean = new BbsBean();
		String saveFolder = "C:/JavaDevProject/Portfolilo/GameInfo/qnaupload";
		...
		this.bbsService.insertqna(bbsbean);
		response.sendRedirect("qna_list.info");
		return null;
	}

	// 수정

	@RequestMapping(value = "/qna_edit.info", method = RequestMethod.GET)
	public ModelAndView editGet(HttpServletRequest request,
		...
		bbsbean = (BbsBean) this.bbsService.qnacheckpwd(no);
		...
		if (session.getAttribute("userstate") == null) {	
			...
			ModelAndView model = new ModelAndView();
			model.setViewName("qna/qna_edit");
	...
	@RequestMapping(value = "/qna_edit.info", method = RequestMethod.POST)
	public ModelAndView edit(HttpServletRequest request,
		...
		this.bbsService.editqna(bbsbean);
		response.sendRedirect("qna_list.info");
		return null;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "qna_list.info", method = RequestMethod.GET)
	public ModelAndView bbs_List(HttpServletRequest request,
		...
		int listcount = this.bbsService.getqnaListCount();

		bbslist = bbsdao.getqnaList(page, limit);
		...
		model2.setViewName("../sub04_01");
		return model2;

	}

	@RequestMapping(value = "/qna_find.info", method = RequestMethod.POST)
	public ModelAndView bbs_find(HttpServletRequest request,
		...
		int listcount = this.bbsService.getqnaFindCount();
		...
		if (find_field.equals("nickname")) {
			findList = this.bbsService.qnaFind(find_name, page, limit);
		} else {
			findList = this.bbsService.qnaFind2(find_name, page, limit);
		}
		...
		model.setViewName("qna/qna_find");
		return model;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/qna_cont.info", method = RequestMethod.GET)
	public ModelAndView bbs_view(HttpServletRequest request,
		...
		// 댓글
		int listcount = this.bbsService.getqnaComCount(no);
		List comlist = new ArrayList();

		request.setAttribute("listcount", listcount);

		comlist = bbsdao.getqnaComList(no);
		...
		bbsbean = (BbsBean) bbsdao.contqna(no);

		request.setAttribute("bbsdata", bbsbean);
		request.setAttribute("comdata", combean);

		ModelAndView model2 = new ModelAndView();
		model2.setViewName("qna/qna_cont");
		return model2;
	}

	@RequestMapping(value = "/qna_del.info", method = RequestMethod.GET)
	public ModelAndView bbs_del(HttpServletRequest request,
		...
		if (session.getAttribute("userstate") == null) {	
			...
			out.println("</script>");
				...
				&& session.getAttribute("userstate").equals("admin")) {
			this.bbsService.delqna(no, checkpwd);
			this.bbsService.qnaComDelAll(no);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/qna_list.info");
			return model;
		} else if (checkpwd.equals(pwd)) {
			this.bbsService.delqna(no, pwd);
			this.bbsService.qnaComDelAll(no);
	...
	@RequestMapping(value = "/qna_com.info", method = RequestMethod.POST)
	public ModelAndView bbs_com(HttpServletRequest request,
		...
		if (nickname == null) {
			...
			this.bbsService.qnaCom(combean);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/qna_cont.info?no=" + no);
			return model;
		}
		return null;
	}

	@RequestMapping(value = "/qna_comdel.info", method = RequestMethod.GET)
	public ModelAndView bbs_comdel(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {

		int bbsno = Integer.parseInt(request.getParameter("bbsno"));
		int comno = Integer.parseInt(request.getParameter("comno"));
		this.bbsService.qnaComDel(bbsno, comno);

		ModelAndView model = new ModelAndView();
		model.setViewName("redirect:/qna_cont.info?no=" + bbsno);
		return model;
	}

	@RequestMapping(value = "/qna_recom.info", method = RequestMethod.GET)
