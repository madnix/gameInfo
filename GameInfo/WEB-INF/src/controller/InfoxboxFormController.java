...
@Controller
public class InfoxboxFormController {
	...
	@RequestMapping(value = "info_xbox_write.info", method = RequestMethod.GET)
	public ModelAndView bbs_write() {

		ModelAndView model = new ModelAndView();
		model.setViewName("info/info_xbox_write");

		return model;
	}

	@SuppressWarnings("unused")
	@RequestMapping(value = "/info_xbox_write.info", method = RequestMethod.POST)
	public ModelAndView submit(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		BbsBean bbsbean = new BbsBean();
		String saveFolder = "C:/JavaDevProject/Portfolilo/GameInfo/infoxboxupload";
		...
		if (UpFile1 != null) {
			...
			String refileName = "info" + year + month + date + random + "."
					+ fileExtension;
		...
		this.bbsService.insertInfoXbox(bbsbean);
		response.sendRedirect("info_xbox_list.info");
		return null;
	}

	// �섏젙

	@RequestMapping(value = "/info_xbox_edit.info", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/info_xbox_edit.info", method = RequestMethod.POST)
	public ModelAndView edit(HttpServletRequest request,
		...
		this.bbsService.editInfoXbox(bbsbean);
		response.sendRedirect("info_xbox_list.info");
		return null;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "info_xbox_list.info", method = RequestMethod.GET)
	public ModelAndView bbs_List(HttpServletRequest request,
		...
		int listcount = this.bbsService.getInfoXboxListCount();

		bbslist = bbsdao.getInfoXboxList(page, limit);
		...
		model2.setViewName("../sub01_02");
		return model2;

	}

	@RequestMapping(value = "/info_xbox_find.info", method = RequestMethod.POST)
	public ModelAndView bbs_find(HttpServletRequest request,
		...
		int listcount = this.bbsService.getInfoXboxFindCount();
		...
		if (find_field.equals("nickname")) {
			findList = this.bbsService.infoXboxFind(find_name, page, limit);
		} else {
			findList = this.bbsService.infoXboxFind2(find_name, page, limit);
		}
		...
		model.setViewName("info/info_xbox_find");
		return model;
	}

	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/info_xbox_cont.info", method = RequestMethod.GET)
	public ModelAndView bbs_view(HttpServletRequest request,
		...
		// �볤�
		int listcount = this.bbsService.getInfoXboxComCount(no);
		List comlist = new ArrayList();

		request.setAttribute("listcount", listcount);

		comlist = bbsdao.getInfoXboxComList(no);

		request.setAttribute("comlist", comlist);
		// �볤���

		bbsdao.infoXboxhit(no);

		bbsbean = (BbsBean) bbsdao.contInfoXbox(no);

		request.setAttribute("bbsdata", bbsbean);
		request.setAttribute("comdata", combean);

		ModelAndView model2 = new ModelAndView();
		model2.setViewName("info/info_xbox_cont");
		return model2;
	}

	@RequestMapping(value = "/info_xbox_del.info", method = RequestMethod.GET)
	public ModelAndView bbs_del(HttpServletRequest request,
		...
		if (session.getAttribute("userstate") != null
				&& session.getAttribute("userstate").equals("admin")) {
			this.bbsService.delInfoXbox(no, checkpwd);
			this.bbsService.infoXboxComDelAll(no);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/info_xbox_list.info");
			return model;
	...
	@RequestMapping(value = "/info_xbox_com.info", method = RequestMethod.POST)
	public ModelAndView bbs_com(HttpServletRequest request,
		...
		if (nickname == null) {
			out.println("<script>");
			out.println("alert('로그인후 작성하실 수 있습니다!')");
			out.println("history.back()");
			out.println("</script>");
		} else {
			combean.setBbsno(no);
			...
			this.bbsService.infoXboxCom(combean);

			ModelAndView model = new ModelAndView();
			model.setViewName("redirect:/info_xbox_cont.info?no=" + no);
			return model;
		}
		return null;
	}

	@RequestMapping(value = "/info_xbox_comdel.info", method = RequestMethod.GET)
	public ModelAndView bbs_comdel(HttpServletRequest request,
			HttpServletResponse response, HttpSession session) throws Exception {

		int bbsno = Integer.parseInt(request.getParameter("bbsno"));
		int comno = Integer.parseInt(request.getParameter("comno"));
		this.bbsService.infoXboxComDel(bbsno, comno);

		ModelAndView model = new ModelAndView();
		model.setViewName("redirect:/info_xbox_cont.info?no=" + bbsno);
		return model;
	}

	@RequestMapping(value = "/info_xbox_recom.info", method = RequestMethod.GET)
	public ModelAndView bbs_recom(HttpServletRequest request,
		...
		this.bbsService.infoXboxRecom(no);
